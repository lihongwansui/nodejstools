...
let SyncUSDTDaemon = async function() {
    ...
    try {
        ...
        await SyncUSDTNodes(base_url, account_name);
...
let SyncUSDTNodes = async function(base_url, account_name) {
    ...
    try {
        ...
        logger.usdtinfo(DebugTag + `Backup ledger index is ${backup_index}`);
        ...
        logger.usdtinfo(DebugTag + `Pending sending amqp list is ${JSON.stringify(traget_trans_list)}`);
...
let GetTargetTransaction = async function(transacton) {
    try {
        ...
        target_trans.address = transacton.SourceTag;
